...
bool insert_ordenado(Aluno *new_student) {
    ...
    if (students.head == NULL) {
        ...
        new_student->ante = new_student->prox = NULL;
        students.size++;
        return true;
    }
    Aluno *atual = students.head->prox;
    while (atual != NULL && new_student->nome >= atual->nome) {
    ...
    int lasts_2_number_for_cpf(Aluno *student){
    ...
    void ler_aluno() {
        ...
        while (getline(file, line)) {
            ...
            Aluno *new_student = new Aluno{
                ...
                colunas[1],
                colunas[2],
                static_cast<float>(atof(colunas[3].c_str())),
                atoi(colunas[4].c_str()),
                colunas[5],
                colunas[6]
            };

            int last2 = lasts_2_number_for_cpf(new_student);
...
void remove_student(Aluno *student){
    ...
    if (student == students.head && student == students.end) {
        ...
        if (students.end != NULL) {
            students.end->prox = NULL;
        } else {
            students.head = NULL;
        }
    }
